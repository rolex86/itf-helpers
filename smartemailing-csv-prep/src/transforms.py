...
def parse_program_codes(programs_raw: str) -> Set[str]:
    if not programs_raw:
        return set()
    parts = [p.strip() for p in programs_raw.split(",")]
...
def apply_country_bucket(df: pd.DataFrame, bucket_cfg: Dict[str, Any]) -> pd.DataFrame:
    out = df.copy()
    cz_sk = set(bucket_cfg.get("cz_sk", []))
    de_at_ch = set(bucket_cfg.get("de_at_ch", []))
    ...
    buckets.loc[countries.isin(cz_sk)] = "CZ_SK"
    buckets.loc[countries.isin(de_at_ch)] = "DE_AT_CH"
