...
_TEMPLATE_NEW_PATH = Path(__file__).with_name("template_new.j2")
_TEMPLATE_NEW_MTIME: Optional[float] = None
_TEMPLATE_NEW_OBJ: Optional[Template] = None
...
def _detect_language(html: str) -> str:
    """
    Detect language from visible text (not raw HTML), so HTML entities do not break detection.
    """
    text = ""
    try:
    ...
    if re.search(r"\b(eigenschaften|funktionen)\b", t) or any(
        x in t for x in [" öffnet", "gerät", "tschechischen republik"]
    ):
        return "de"
    if re.search(r"\b(features|functions)\b", t) or any(
        x in t for x in [" rugged", " device", " barcode", "opens in a new tab"]
    ):
        return "en"
    return "cs"
...
def _collect_intro_html(soup: BeautifulSoup) -> str:
    ...
    if isinstance(start, Tag):
        iterator = start.find_all_next(["p", "ul", "ol", "div"], limit=220)
    else:
        root = soup.body if isinstance(soup.body, Tag) else soup
        iterator = root.find_all(["p", "ul", "ol", "div"], limit=220)

    for el in iterator:
        txt = el.get_text(" ", strip=True)

        low_txt = (txt or "").strip().lower()

        # Stop hned, pokud blok začíná "Vlastnosti/Funkce/..." (typicky začátek features section)
        if any(
            low_txt == w.lower()
            or low_txt.startswith(w.lower() + " ")
            or low_txt.startswith(w.lower() + ":")
            for w in stop_words
        ):
            break

        # Původní "měkký" stop (pro krátké nadpisy někde v textu)
        if _has_stop_header(txt) and len(low_txt) < 120:
            break
...
_FEATURE_RULES = [(key, re.compile(pat, flags=re.I)) for key, pat in _FEATURE_RULES_RAW]
_FUNCTION_RULES = [(key, re.compile(pat, flags=re.I)) for key, pat in _FUNCTION_RULES_RAW]
...
def _has_tile_layout_signals(soup: BeautifulSoup) -> bool:
    has_h2 = soup.find("h2") is not None
    if not has_h2:
        return False
    ...
    for d in soup.find_all("div"):
        st = (d.get("style") or "").replace(" ", "").lower()
        if "--width_max:60%" in st:
            has_60 = True
        if "--width_max:40%" in st:
            has_40 = True
        if has_60 and has_40:
            return True
    return False
...
def _parse_new_template(soup: BeautifulSoup, lang: str, include_tiles: Optional[bool] = True) -> Extracted:
    ui = _ui(lang)
    ...
    if isinstance(pwrap, Tag):
        section_blocks: List[Tuple[str, List[CardItem]]] = []

        for bar in pwrap.find_all(
            lambda t: isinstance(t, Tag)
            and t.name == "div"
            and "linear-gradient(90deg" in (t.get("style") or "")
        ):
            st = bar.find("strong")
            if not st:
                continue
            ...
            col = bar.find_parent("div")
            if not isinstance(col, Tag):
                continue

            items: List[CardItem] = []
            for card in col.find_all(
                lambda t: isinstance(t, Tag)
                and t.name == "div"
                and "flex: 1 1 280px" in (t.get("style") or "")
            ):
            ...
            if items:
                section_blocks.append((col_title, items))

        if section_blocks:
            for idx, (col_title, items) in enumerate(section_blocks):
    ...
    # --- NOTE (editable/preserved on round-trip) ---
    note_html = _fixed_note_html(lang)
    nwrap = soup.find(id="sid-c5-note")
    if isinstance(nwrap, Tag):
        ncard = nwrap.find("div")
        source = ncard if isinstance(ncard, Tag) else nwrap
        candidate = _safe_decode(source)
        if _strip_ws(source.get_text(" ", strip=True)):
            note_html = candidate

    tiles = _extract_tiles_c66_style(soup) if include_tiles is not False else []
...
def _get_new_template() -> Template:
    global _TEMPLATE_NEW_MTIME, _TEMPLATE_NEW_OBJ

    mtime = _TEMPLATE_NEW_PATH.stat().st_mtime
    if _TEMPLATE_NEW_OBJ is None or _TEMPLATE_NEW_MTIME != mtime:
        tpl_src = _TEMPLATE_NEW_PATH.read_text(encoding="utf-8")
        _TEMPLATE_NEW_OBJ = Template(tpl_src)
        _TEMPLATE_NEW_MTIME = mtime
    return _TEMPLATE_NEW_OBJ


def render_new_template(model: Extracted) -> str:
    if not (model.note_html or "").strip():
        model.note_html = _fixed_note_html(model.language)
    model.has_note = True

    tpl = _get_new_template()

    ui = _ui(model.language)

    highlights = [
        {"kicker": (h.label or "").strip(), "value": (h.value or "").strip()}
        for h in (model.highlights or [])
        if (h.label or "").strip() and (h.value or "").strip()
    ]
    features_list = [
        {"label": (x.label or "").strip(), "value": (x.value or "").strip()}
        for x in (model.features or [])
        if (x.label or "").strip() and (x.value or "").strip()
    ]
    functions_list = [
        {"label": (x.label or "").strip(), "value": (x.value or "").strip()}
        for x in (model.functions or [])
        if (x.label or "").strip() and (x.value or "").strip()
    ]
    ...
    sections = {
        ...
        "params": bool(features_list or functions_list),
        "tiles": bool(model.tiles),
        "note": True,
    }
    sections["hero"] = bool(
        sections["intro"]
        or sections["highlights"]
        or sections["spin3d"]
        or (model.hero_img_src or "").strip()
    )
...
def convert_old_html_to_c5(
    old_html: str,
    include_tiles: Optional[bool] = True,
    ...
    if include_tiles is False:
        tiles = []
    elif include_tiles is True:
        tiles = _extract_tiles_c66_style(soup)
    else:
        tiles = _extract_tiles_c66_style(soup) if _has_tile_layout_signals(soup) else []
